...
public class CommonResponseUtil {
     ...
     * 构建含有分页信息的成功应答
     *
     * @return CommonResponse<CommonPage < T>>
     **/
    public static <T> CommonResponse<CommonPage<T>> success(CommonPage.PageInfo pageInfo, List<T> list) {
        // 创建分页应答类型
        CommonResponse<CommonPage<T>> commonResponse = new CommonResponse<>();
        // 创建分页应答data类型
        CommonPage<T> commonPage = new CommonPage<>(pageInfo, list);
        commonResponse.setData(commonPage);
        commonResponse.setCode(COMMON_RESPONSE_CODE_SUCCESS);
        commonResponse.setMessage(COMMON_RESPONSE_MESSAGE_SUCCESS);
        return commonResponse;
    }
